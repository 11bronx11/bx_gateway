...
**定位:单机轻量化高性能 L7 网关。** 静态配置驱动，安全策略前置(名单/WAF/鉴权/限流按序短路，拒绝的请求不触达上游)，连接、路由、上游状态、风险规则、观测数据共用同一份运行时事实。"高性能"来自设计和实测:I/O 等待让协程让出不占线程、请求级 `shared_ptr` 快照隔离在途流量、Guard 原子快照零锁只读、请求响应体流式转发不整体缓冲、CPU 活 offload 出 reactor;
...
目标场景：单机或一组 Linux 主机，资源受限（单核到 2C2G），有一定并发量，同时需要
路由、IP 动态封禁、认证鉴权、限流、健康检查、熔断、负载均衡、连接池，这些基本能力，并且要求可观测、
可全链路追踪、可控的 API 入口。这是 Bronx 的设计立场，也是本次对比测试的焦点。

测试对象：Bronx（`gw + hub`，即网关和控制面合计计入资源）、Spring Cloud Gateway、Kong
DB-less、Kong + PostgreSQL。四个产品在同一台机器上严格串行运行，配置按各产品原生最佳实践，
主流方案，常见实现冻结，公平配置，不做针对优化，更不故意做弱，确保结果可信。

#### 综合测试：单核三档内存，混合请求链

测试案例：`full-5m`。单核（`CPUQuota=100%`），三档内存（512 MiB / 1 GiB / 2 GiB），固定
1000 RPS 目标，60 秒预热后 300 秒正式窗口。请求不是裸转发——按以下比例混合：

- 70% 正常请求（路由、负载均衡、连接池、请求头改写）
- 10% 认证请求（JWT 签名 / 过期 / scope 校验）
- 5% WAF 请求（SQLi / XSS 特征，预期 403）
- 5% IP 封禁请求（已封 IP，预期 403）
- 5% 限流请求（超桶速率，预期 429 后恢复放行）
- 5% 路由测试（验证 prefix 改写、header 注入）

全程检查响应合同：状态码、上游实例名、方法、改写后路径、配置版本头、内部头删除、request-id
端到端一致性。正常和路由请求的正确性判断独立于故障窗口语义，不会被总体成功率平均。

上游五台受控 mock，正式窗口中依次注入：

| 上游 | 正常 | 故障行为 | 观察项 |
|---|---|---|---|
| A / B | 正常响应，1-2 ms | 无故障 | 主流量、连接复用、健康检查 |
| C | 正常响应 | 90-108 s：返回 503 | 被动隔离、熔断可见性 |
| D | 正常响应 | 112-130 s：慢响应注入 | 超时治理、尾延迟 |
| E | 正常响应 | 134-152 s：下线 → 恢复 | 主动健康检查摘除与回填 |

正式窗口还包含：有效/无效 reload（60 / 75 s）、IP-ban 规则变化（180 / 195 s）、Hub 控制面
重启（150 s）、高精度 trace 开关（195-225 s）、日志轮转（240 s），以及 100 个持续 WebSocket
客户端。同期检测观测探针和事件合规性。

| 资源档 | 产品 | 网关 RPS | 内存峰值 | p50 | p99 | 语义正确率 | 故障可用率 | 恢复可用率 |
|---|---|---:|---:|---:|---:|---:|---:|---:|
| 512 MiB | Bronx | 999.66 | 163.7 MiB | 3.63 ms | 59.88 ms | 100.000% | 99.975% | 99.973% |
| 512 MiB | Kong DB-less | 976.83 | 512.0 MiB | 3.60 ms | 374.00 ms | 99.740% | 99.706% | 100.000% |
| 512 MiB | Kong PostgreSQL | 988.83 | 477.1 MiB | 2.69 ms | 265.23 ms | 99.983% | 99.861% | 100.000% |
| 512 MiB | Spring | 987.77 | 357.6 MiB | 4.73 ms | 603.12 ms | 92.185% | 73.226% | 92.475% |
| 1 GiB | Bronx | 998.18 | 161.9 MiB | 3.71 ms | 76.32 ms | 100.000% | 99.978% | 99.973% |
| 1 GiB | Kong DB-less | 986.17 | 768.7 MiB | 2.63 ms | 192.62 ms | 99.997% | 99.853% | 99.925% |
| 1 GiB | Kong PostgreSQL | 982.17 | 722.5 MiB | 2.99 ms | 353.87 ms | 99.840% | 99.712% | 100.000% |
| 1 GiB | Spring | 961.54 | 360.4 MiB | 4.68 ms | 780.49 ms | 93.741% | 59.513% | 80.451% |
| 2 GiB | Bronx | 999.24 | 146.4 MiB | 3.51 ms | 36.50 ms | 100.000% | 99.978% | 99.973% |
| 2 GiB | Kong DB-less | 997.97 | 589.0 MiB | 2.39 ms | 82.07 ms | 100.000% | 99.827% | 100.000% |
| 2 GiB | Kong PostgreSQL | 973.19 | 780.3 MiB | 2.64 ms | 455.87 ms | 99.186% | 99.849% | 100.000% |
| 2 GiB | Spring | 995.76 | 339.8 MiB | 3.15 ms | 157.35 ms | 98.538% | 75.666% | 95.716% |

语义正确率 = 所有请求中响应合同合规比例（含预期 403/429）；故障可用率 = 故障注入窗口内
HTTP 200 比例；恢复可用率 = 故障解除后 HTTP 200 比例。p50/p99 只统计正常请求延迟。

#### 2 核 2 GiB 健康容量测试

测试案例：`formal`。2 核（`CPUQuota=200%`）、2 GiB 总预算，两台健康上游始终等权，
无故障注入，持续升压观察稳定容量上限。

每档 8 秒，依次递进：500 → 1000 → 2000 → 4000 → 8000 → 16000 → 32000 RPS，每个产品连续
执行 3 轮独立测量。某档同时满足以下全部条件才计为该轮稳定：实际 RPS ≥ 目标 99%、P99 ≤
100 ms、429 = 0、非预期响应 = 0、dropped_iterations ≤ 1%。**最高稳定 RPS 要求三轮全部
稳定**，不取单次最好值。

请求全程经过路由匹配、客户端 IP 解析、IP 策略、WAF 扫描、JWT 验签/scope 校验、限流、
请求头改写、负载均衡、连接池和转发；访问日志与 Prometheus 指标开启，高精度 trace 关闭。

**各产品在不同负载阶梯的表现**：

| 产品 | 档位 RPS | P99 均值 | 内存峰值均值 | 三轮稳定 | 尝试轮次 |
|---|---:|---:|---:|:---:|:---:|
| Bronx | 500 | 4.19 ms | 44.8 MiB | ✓ | 3 |
| Bronx | 1000 | 7.81 ms | 47.5 MiB | ✓ | 3 |
| Bronx | 2000 | 4.54 ms | 53.0 MiB | ✓ | 3 |
| Bronx | 4000 | 29.69 ms | 66.0 MiB | ✗ | 3 |
| Bronx | 8000 | 236.76 ms | 73.4 MiB | ✗ | 1 |
| Kong DB-less | 500 | 10.35 ms | 479.1 MiB | ✓ | 3 |
| Kong DB-less | 1000 | 7.32 ms | 497.5 MiB | ✓ | 3 |
| Kong DB-less | 2000 | 13.22 ms | 538.4 MiB | ✓ | 3 |
| Kong DB-less | 4000 | 64.13 ms | 616.5 MiB | ✗ | 3 |
| Kong PostgreSQL | 500 | 28.59 ms | 424.3 MiB | ✓ | 3 |
| Kong PostgreSQL | 1000 | 12.35 ms | 442.2 MiB | ✗ | 3 |
| Kong PostgreSQL | 2000 | 8.64 ms | 518.1 MiB | ✗ | 2 |
| Kong PostgreSQL | 4000 | 78.47 ms | 595.6 MiB | ✗ | 2 |
| Spring | 500 | 47.95 ms | 352.6 MiB | ✗ | 3 |
| Spring | 1000 | 18.81 ms | 335.7 MiB | ✗ | 1 |
| Spring | 2000 | 118.09 ms | 337.9 MiB | ✗ | 1 |

**最高稳定容量汇总**：

| 产品 | 最高稳定 RPS（三轮全稳） | 整体内存峰值均值 |
|---|---:|---:|
| Bronx | 2000 | 79.7 MiB |
| Kong DB-less | 2000 | 616.5 MiB |
| Kong PostgreSQL | 500 | 521.4 MiB |
| Spring | < 500 | 353.4 MiB |

Kong PostgreSQL 在 1000 RPS 档三轮均未同时满足稳定条件（P99 超限或非预期响应），最高稳定
档位为 500 RPS。Spring 在三轮 500 RPS 中均出现非预期语义响应，未达稳定基准；三轮均停在
500 RPS 档，最高稳定 RPS 记为 0。两者的容量上限结论限定在本次 2C2G 配额和冻结配置下，不
外推至其他资源规格或配置调优后的产品能力。

#### 场景边界与结论

上述数据支持一个有限判断：在单机、资源受限（单核到 2C2G）、完整网关能力同时开启（路由、
鉴权、封禁、限流、熔断、健康检查、可观测）的入口场景下，Bronx 能以较低的内存峰值（三档均在
170 MiB 以内）交付接近目标 RPS，尾延迟保持在合理范围，并在故障注入和控制面事件中维持 >99.9%
故障可用率。

这不是对 Kong 或 Spring Cloud Gateway 的全面产品排名，不包括 TLS / HTTP/2 / 服务发现 /
插件生态或跨机生产长稳结论。Nginx 纯代理的单核吞吐和 CPU 效率明显更强，Bronx 对标的是
“需要网关业务能力的轻量受控入口”，不是替代 Nginx 或大型 API 管理平台。

详细测试设计、能力口径、请求比例、上游行为时间线、通过条件和结果目录：

- [综合测试说明（单核三档内存）](benchmark/api_gateway_comparison/repro/README.md)
- [2C2G 健康容量套件](benchmark/api_gateway_comparison/capacity_2c2g/README.md)
